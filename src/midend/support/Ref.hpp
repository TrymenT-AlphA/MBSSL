...
#include "midend/support/Casting.hpp"
...
private:
  void inc() const {
    if (ptr) ++ptr->_refCount.refCount;
  }

  void dec() const {
    if (ptr && --ptr->_refCount.refCount == 0) delete ptr;
  }
...
public:
  ...
  ref<T> &operator=(ref<U> &&r) noexcept {
    if (static_cast<void *>(this) == static_cast<void *>(&r)) return *this;
