...
inline unsigned indexOfSingleBit(uint64_t x) {
  ...
  if (x & (UINT64_C(0xFFFFFFFF) << 32)) res += 32;
  assert(res < 64);
...
[[nodiscard]] static constexpr inline auto countLeadingZeroes(T &&x) noexcept
    -> std::enable_if_t<!std::numeric_limits<std::decay_t<T>>::is_signed &&
                            std::numeric_limits<std::decay_t<T>>::digits == std::numeric_limits<unsigned>::digits,
                        int> {
  assert(x > 0);
...
[[nodiscard]] static constexpr inline auto countLeadingZeroes(T &&x) noexcept
    -> std::enable_if_t<!std::numeric_limits<std::decay_t<T>>::is_signed &&
                            ...
                            std::numeric_limits<unsigned>::digits != std::numeric_limits<unsigned long>::digits,
                        int> {
  assert(x > 0);
...
template <typename T>
[[nodiscard]] static constexpr inline auto countLeadingZeroes(T &&x) noexcept -> std::enable_if_t<
    !std::numeric_limits<std::decay_t<T>>::is_signed &&
        std::numeric_limits<std::decay_t<T>>::digits == std::numeric_limits<unsigned long long>::digits &&
        std::numeric_limits<unsigned>::digits != std::numeric_limits<unsigned long long>::digits &&
        std::numeric_limits<unsigned long>::digits != std::numeric_limits<unsigned long long>::digits,
    int> {
  assert(x > 0);
...
template <typename T>
[[nodiscard]] static constexpr inline auto countTrailingZeroes(T &&x) noexcept -> std::enable_if_t<
    !std::numeric_limits<std::decay_t<T>>::is_signed &&
        std::numeric_limits<std::decay_t<T>>::digits == std::numeric_limits<unsigned long long>::digits &&
        std::numeric_limits<unsigned>::digits != std::numeric_limits<unsigned long long>::digits &&
        std::numeric_limits<unsigned long>::digits != std::numeric_limits<unsigned long long>::digits,
...
[[nodiscard]] static constexpr inline std::size_t roundUpToMultipleOf4096(std::size_t const x) {
