#ifndef __SPARSESTORAGE_HPP__
#define __SPARSESTORAGE_HPP__
...
public:
  SparseStorage(const ValueType &defaultValue = ValueType()) : defaultValue(defaultValue) {
  }

  SparseStorage(const std::unordered_map<size_t, ValueType> &internalStorage, const ValueType &defaultValue)
  ...
  SparseStorage(const std::vector<ValueType> &values, const ValueType &defaultValue = ValueType())
      : defaultValue(defaultValue) {
  ...
  void store(size_t idx, InputIterator iteratorBegin, InputIterator iteratorEnd) {
  ...
  const ValueType &defaultV() const {
    return defaultValue;
  };
...
SparseStorage<unsigned char> sparseBytesFromValue(const U &value) {
  const unsigned char *valueUnsignedCharIterator = reinterpret_cast<const unsigned char *>(&value);
  SparseStorage<unsigned char> result;
  result.store(0, valueUnsignedCharIterator, valueUnsignedCharIterator + sizeof(value));
  return result;
}
