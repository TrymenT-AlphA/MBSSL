...
#include "midend/expr/SymbolicSource.hpp"
#include "midend/support/Bits.hpp"
...
protected:
  struct ExprHash {
    unsigned operator()(Expr *const e) const {
      return e->hash();
    }
  };
...
struct Expr::CreateArg {
  ...
  CreateArg(ref<Expr> e)
      : expr(e), width(Expr::InvalidWidth), rm(llvm::APFloat::rmNearestTiesToEven), _isRoundingMode(false) {
  }

  CreateArg(llvm::APFloat::roundingMode _rm) : expr(0), width(Expr::InvalidWidth), rm(_rm), _isRoundingMode(true) {
  }
...
public:
  Array(ref<Expr> _size, const ref<SymbolicSource> source, Expr::Width _domain = Expr::Int32,
        Expr::Width _range = Expr::Int8, unsigned _id = 0);

public:
  bool isSymbolicArray() const {
    return !isConstantArray();
  }
...
public:
  ...
  static ref<Expr> create8(const ref<Expr> &kid1, const ref<Expr> &kid2, const ref<Expr> &kid3, const ref<Expr> &kid4,
                           const ref<Expr> &kid5, const ref<Expr> &kid6, const ref<Expr> &kid7, const ref<Expr> &kid8);
...
#define FP_CAST_EXPR_CLASS(_class_kind)                                                                          \
  ...
  public:                                                                                                        \
    ...
    const llvm::APFloat::roundingMode roundingMode;                                                              \
                                                                                                                 \
  public:                                                                                                        \
    _class_kind##Expr(ref<Expr> e, Width w, llvm::APFloat::roundingMode rm) : CastExpr(e, w), roundingMode(rm) { \
    }                                                                                                            \
  ...
  protected:                                                                                                     \
    virtual int compareContents(const Expr &b) const {                                                           \
      ...
      if (roundingMode != eb.roundingMode) return roundingMode < eb.roundingMode ? -1 : 1;                       \
      return 0;                                                                                                  \
    }                                                                                                            \
  };
...
#define FLOAT_ARITHMETIC_EXPR_CLASS(_class_kind)                                                           \
  ...
  public:                                                                                                  \
    ...
    const llvm::APFloat::roundingMode roundingMode;                                                        \
                                                                                                           \
  public:                                                                                                  \
    _class_kind##Expr(const ref<Expr> &l, const ref<Expr> &r, const llvm::APFloat::roundingMode rm)        \
        : BinaryExpr(l, r), roundingMode(rm) {                                                             \
    }                                                                                                      \
    static ref<Expr> alloc(const ref<Expr> &l, const ref<Expr> &r, const llvm::APFloat::roundingMode rm) { \
...
#define FP_UNARY_ARITHMETIC_EXPR_CLASS(_class_kind)                                                           \
  ...
  public:                                                                                                     \
    ...
    const llvm::APFloat::roundingMode roundingMode;                                                           \
    ref<Expr> expr;                                                                                           \
    static ref<Expr> alloc(const ref<Expr> &e, const llvm::APFloat::roundingMode rm) {                        \
      ref<Expr> r(new _class_kind##Expr(e, rm));                                                              \
      r->computeHash();                                                                                       \
      r->computeHeight();                                                                                     \
      return createCachedExpr(r);                                                                             \
    }                                                                                                         \
    static ref<Expr> create(const ref<Expr> &e, const llvm::APFloat::roundingMode rm);                        \
                                                                                                              \
    Width getWidth() const {                                                                                  \
      return expr->getWidth();                                                                                \
    }                                                                                                         \
    ...
    static bool classof(const _class_kind##Expr *) {                                                          \
      return true;                                                                                            \
                                                                                                              ...
                                                                                                              \
  private:                                                                                                    \
    _class_kind##Expr(const ref<Expr> &e, const llvm::APFloat::roundingMode rm) : roundingMode(rm), expr(e) { \
    }                                                                                                         \
  };
...
public:
  ...
  static ref<Expr> either(const ref<Expr> &e0, const ref<Expr> &e1);
  static bool classof(const Expr *E) {
    return E->getKind() == Expr::FAbs;
  }
  static bool classof(const FAbsExpr *) {
    return true;
  }
...
#ifndef NDEBUG
    if (w <= 64) assert(v == bits64::truncateToNBits(v, w) && "invalid constant");
#endif
    return alloc(v, w);
  }
...
inline bool Expr::isOne() const {
  if (const ConstantExpr *CE = dyn_cast<ConstantExpr>(this)) return CE->isOne();
  return false;
}
